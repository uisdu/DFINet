...
def get_args():
    ...
    parser.add_argument("--checkpoint", type=str,
                        default="/home/LC/checkpoint.pth",
    ...
    parser.add_argument("--save_csv", type=str,
                        default="/home/LC/test_results.csv",
...
if __name__ == "__main__":
    ...
    for iter in range(num_iter):
        ...
        for i, (x, y) in tqdm(enumerate(zip(test_x, test_y)), total=len(test_x)):
            ...
            ## Prev mask
            pmask = prev_masks[i]                  # np.ndarray(H,W)
            pmask = torch.from_numpy(pmask).float()
            pmask = pmask.unsqueeze(0).unsqueeze(0).to(device)
            ...
            with torch.no_grad():
                ...
                prev_masks[i]  = pred_y[0, 0].cpu().numpy()    # [H,W]
                # pred_bin = (pred_np > 0.5).astype(np.float32)

                             # 用于下一轮
                tmp_masks.append(prev_masks[i])
