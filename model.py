...
from blocks import ResidualBlock,MADRB,HPPM,DSFM,DFFFM
...
class EncoderBlockl(nn.Module):
    def __init__(self, in_c, out_c, name=None):
        super(EncoderBlockl, self).__init__()
        ...
        self.p1 = DFFFM(out_c, out_c)
        self.pool = nn.MaxPool2d((2, 2))
        self.e5=  HPPM(256,(20,12),norm_layer=nn.BatchNorm2d,up_kwargs = {'mode': 'bilinear', 'align_corners': True})
        self.e6=  HPPM(256,(20,12),norm_layer=nn.BatchNorm2d,up_kwargs = {'mode': 'bilinear', 'align_corners': True})
...
if __name__ == "__main__":
    x = torch.randn((1, 3, 640, 512)).cuda(1)
    m = torch.randn((1, 1, 640, 512)).cuda(1)
    model = DFINet().cuda(1)
    flops, params = profile(model, inputs=([x,m], ))
